...
class FireStoreService {
  ...
  Future<void> editTodoFire(TodoFire todofire, String newTitle) async {
    await  db.collection('todos').doc(todofire.todoId).update(todofire.copyWith(title: newTitle).toJson());
  }

  Future<void> toggleTodoCompletion(TodoFire todofire) async {
    await  db.collection('todos').doc(todofire.todoId).update(todofire.copyWith(isCompleted: !todofire.isCompleted).toJson());
  }
